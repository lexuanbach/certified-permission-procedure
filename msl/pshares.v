...
Proof.
  intros.
  change (Share.rel (proj1_sig sh) Share.leftTree) with (fst (Share.split (proj1_sig sh))).
...
Proof.
  intros.
  change (Share.rel (proj1_sig sh) Share.rightTree) with (snd (Share.split (proj1_sig sh))).
  case_eq (Share.split (proj1_sig sh)); simpl.
  intros.
  generalize (split_nontrivial' _ _ _ H); intro.
  intros ? ?. apply unit_identity in H1. destruct sh; simpl in H0.
...
Proof.
  ...
  inversion H.
  apply injective_projections; auto. inv H.
  apply injective_projections; apply lifted_eq;simpl in *.
